...
Interactive hilaal/crescent-visibility explorer for comparing how Moon/Sun geometry changes by
...
country/state/capital/city overlays, map tooltips, Yallop/Odeh criteria, charts, Islamic
sighting-gate fields, and the full documentation tab. It is embedded at
`https://abdullahhussain.com.au/hilaal/`.

## Accuracy Note

The app uses real Skyfield astronomy calculations for topocentric Sun/Moon geometry. It implements
formula-based Yallop and Odeh ARCV/crescent-width criteria. Other placeholder metrics are removed
from the public app until they are backed by real sourced implementations.
...
- Can use the browser's current location, date, time, and time zone.
- Provides live address suggestions for arbitrary city/suburb/address queries when the geocoder is
  reachable.
...
- Shows tooltips with Moon altitude, Sun altitude, elongation, ARCV, DAZ, crescent width,
  Yallop q, Odeh V, illumination, Moon age, UTC sample time, and the best UTC hour for date-best
  maps.
...
- `models.py`: Yallop/Odeh visibility threshold helpers.
